...
export function HorizontalScrollText({
  ...
  return (
    <section
      id="horizontal-scroll-text"
      className={cn("relative bg-background py-8", className)}
    >
      <ParallaxText textClassName={textClassName} baseVelocity={-5}>
        {children}
      </ParallaxText>
    </section>
  );
...
function ParallaxText({
  ...
  return (
    <div
      ref={containerRef}
      className="left-0 top-0 m-0 flex flex-nowrap overflow-hidden"
    >
      <motion.div
        className={cn(
          "flex flex-nowrap whitespace-nowrap text-2xl font-bold uppercase leading-6 tracking-widest md:text-5xl",
          textClassName,
        )}
        style={{ x }}
      >
