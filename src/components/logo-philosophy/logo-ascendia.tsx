...
interface LogoAscendiaBaseProps {
  className?: string;
  isGrayScale?: boolean;
}

export function LogoAscendiaBase({
  className,
  isGrayScale = false,
}: LogoAscendiaBaseProps) {
  return (
    <motion.div
      className={cn("aspect-square w-64", className)}
      animate={isGrayScale ? "gray-scale" : "no-gray-scale"}
      variants={{
        "gray-scale": {
...
export function LogoAscendiaGear({ className }: LogoAscendiaBaseProps) {
  ...
  return (
    <div className={cn("relative aspect-square w-64", className)} ref={divRef}>
      <motion.svg
      ...
      <LogoAscendiaBase isGrayScale={isInView} />
    </div>
  );
}

export function LogoAscendiaNetwork({ className }: LogoAscendiaBaseProps) {
  const divRef = useRef(null);
  const isInView = useInView(divRef, {
    amount: 1,
    once: true,
...
export function LogoAscendiaWing({ className }: LogoAscendiaBaseProps) {
  const divRef = useRef(null);
  const isInView = useInView(divRef, {
    amount: 1,
    once: true,
...
export function LogoAscendiaGaruda({ className }: LogoAscendiaBaseProps) {
  const divRef = useRef(null);
  const isInView = useInView(divRef, {
    amount: 1,
    once: true,
